...
FROM W_LOAN_DAILY_F LD

--LEFT JOIN (SELECT DISTINCT LOAN_WID, SHOP_WID, CHANNEL_WID 
--				FROM W_LOAN_OFFICE_F) LOF ON LD.LOAN_WID = LOF.LOAN_WID 
	LEFT JOIN W_SHOP_D S ON LD.SHOP_WID = S.SHOP_WID 
--	LEFT JOIN W_CHANNEL_D CD ON LOF.CHANNEL_WID = CD.CHANNEL_WID 
	LEFT JOIN W_ASSET_TP_D ATP ON LD.ASSET_TYPE_WID = ATP.ASSET_TP_WID 
	LEFT JOIN 
	F88DWH.W_LOAN_DAILY_F LD1 ON LD.LOAN_WID = LD1.LOAN_WID 
							  AND LD.SCHEDULE_NEXT_DATE = TO_DATE(LD1.DATE_WID, 'YYYY MM DD')
	LEFT JOIN W_LOAN_DTL_F LDT ON LD.LOAN_WID = LDT.LOAN_WID 
	LEFT JOIN W_EMPLOYEE_D F ON LD1.SUPPORTER_WID = F.EMPLOYEE_WID 
...
WHERE 
	...
	AND LD.LOAN_CODE NOT IN (SELECT B.LOAN_CODE FROM INCENTIVE_DELETE_LOAN_CODE B WHERE LOAI='DU NO QUA HAN' AND MONTH_ID='202212')
